...
const TransactionModal = (expense = true) => {
  ...
  bloco.querySelector("#button-save").addEventListener("click", () => {
    ...
    if (repetitionActive) {
      const radioFixed = form.querySelector(
        'input[name="recurrent"][value="fixed"]'
      );
      const radioInstallments = form.querySelector(
        'input[name="recurrent"][value="installments"]'
      );
      ...
      if (radioFixed.checked) {
        ...
        if (!frequencyAlt) {
          displayError(
            "#input-frequency-alt",
            "O período para receita fixa é obrigatório."
          );
        }
      } else if (radioInstallments.checked) {
        const frequencyInstallments =
          form.querySelector("#input-frequency").value;
        const numberValue = form.querySelector("#input-number").value;
        if (!frequencyInstallments) {
          displayError(
            "#input-frequency",
            "O período para receita parcelada é obrigatório."
          );
        }
        if (!numberValue) {
        ...
        displayError(
          "#recurrent-installments",
          "Selecione o tipo de repetição desejado."
        );
      }
    }

    const observationActive =
      form.querySelector("#observation").value === "true";
    if (observationActive) {
      const observationContent = form
        .querySelector("#input-observation")
        .value.trim();
    ...
    for (const key of [...data.keys()]) {
      ...
      if (
        !repetitionActive &&
        ["number", "frequency-installments", "frequency-fixed"].includes(key)
      ) {
        data.delete(key);
      }
    }
    ...
    const dataObj = Object.fromEntries(data.entries());
    console.log(dataObj);
    addTransaction(expense ? "expense" : "income", dataObj);
    bloco.remove();
  });
